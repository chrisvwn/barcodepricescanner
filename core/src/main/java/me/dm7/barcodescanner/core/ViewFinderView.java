...
public class ViewFinderView extends View implements IViewFinder {
    ...
    protected Paint mROIPaint;
    protected int mBorderLineLength;
    protected boolean mSquareViewFinder;
    private boolean mIsLaserEnabled;
    private float mBordersAlpha;
    private int mViewFinderOffset = 0;
    private ArrayList<BCROI> mROI;
    ...
    private void init() {
        ...
        //ROI paint
        mROIPaint = new Paint();
        mROIPaint.setColor(0xffff00);
        mROIPaint.setStyle(Paint.Style.STROKE);
        mROIPaint.setStrokeWidth(6);
        mROIPaint.setAntiAlias(true);
        mROIPaint.setTextSize(2);
        mROIPaint.setAlpha(255);
    }
    ...
    public void setROI(ArrayList<BCROI> ROI) {
        mROI = ROI;
    }
    ...
    public void drawROI(Canvas canvas) {

        if(mROI != null && mROI.size() > 0) {
            for(int i = 0; i < mROI.size(); i++) {
                org.opencv.core.Rect b = mROI.get(i).bRect;

                canvas.drawRect(b.x,b.y, b.x+b.width, b.y+b.height, mBorderPaint);
                canvas.drawText(mROI.get(i).value, b.x, b.y+2, mBorderPaint);
            }
        }
    }
...
//        mFramingRect = new Rect(0,0, viewResolution.x, viewResolution.y);
    }
}
